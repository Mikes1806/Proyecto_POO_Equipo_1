...
class Gestorperfil:
    ...
    def _add_info(self, df_mi_perfil:dict) -> None:
        ...
        if st.button("Agregar infomación"):
            if not str(monto_mensual).replace('.', '', 1).isdigit() or float(monto_mensual) < 0:
                st.error("El monto mensual debe ser un número positivo.")
            elif not str(monto_mensual_extra).replace('.', '', 1).isdigit() or float(monto_mensual_extra) < 0:
                st.error("El monto mensual extra debe ser un número positivo.")
            elif not str(numero_telefono).replace('.', '', 1).isdigit() or float(numero_telefono) < 0:
                st.error("El número de teléfono es inválido.")
            else:
                monto_mensual_total = float(monto_mensual) + float(monto_mensual_extra)
                new_record = {
                    ...
                    "monto_mensual_total": monto_mensual_total,
                }
    ...
    def _eliminar_perfil(self, df_mi_perfil:dict) -> None:
    ...
    def _displey_perfil(self, df_mi_perfil:dict) -> None:
        ...
        monto_mensual_total = df_mi_perfil ['monto_mensual_total'][0]
        ...
        st.markdown(
            ...
            <div style="
                ...
                <p style='color: white; margin: 8px;'>  {nombre_user}</p>
            </div>
            """,
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding: 2px;
                background-color: #151516;">
                <p style='color: white; margin: 8px;'>  {edad_seleccionada}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding: 2px;
                background-color: #151516;">
                <p style='color: white; margin: 8px;'>  {numero_telefono}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding: 2px;
                background-color: #151516;">
                <p style='color: white; margin: 8px;'>  {correo_electronico}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding: 2px; 
                background-color: #151516;">
                <p style='color: white; margin: 8px;'> {monto_mensual}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding: 2px;
                background-color: #151516;">
                <p style='color: white; margin: 8px;'> {monto_mensual_extra}</p>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    def _modify_row(self, df_mi_perfil:dict) -> None:
        ...
        if not df_mi_perfil.empty:
            ...
            if selected_index is not None:
                ...
                with col2:
                    ...
                    edad = st.selectbox(
                        "Edad:",
                        edades,
                        index=edades.index(int(selected_record["edad"])),
                        key="edad",
                    )
                ...
                if st.button("Guardar Cambios", key="save_button"):
                    if not str(monto_mensual).replace('.', '', 1).isdigit() or float(monto_mensual) < 0:
                        ...
                        df_mi_perfil.at[selected_index, "monto_mensual"] = float(monto_mensual)
                        df_mi_perfil.at[selected_index, "monto_mensual_extra"] = float(monto_mensual_extra)
                        df_mi_perfil.at[selected_index, "monto_mensual_total"] = monto_mensual_total
                        df_mi_perfil.to_csv(
                            "/usr/src/app/app/classes/logics/data/mi_perfil.csv",
                            mode="w",
                            index=False,
            ...
            st.error("No existen datos disponibles para modificar.")
